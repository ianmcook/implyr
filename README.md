...
**implyr** is a SQL backend to [dplyr](https://cran.r-project.org/package=dplyr) for [Apache Impala (incubating)](https://impala.incubator.apache.org/), the massively parallel processing query engine for Apache Hadoop.
...
Initialize the Java Virtual Machine (JVM) by calling `.jinit()` and passing a vector containing the paths to all the Impala JDBC driver JAR fies as the `classpath` argument:
...
implyr does not provide tools for loading local data into Impala tables. This is because Impala can query data stored in several different filesystems and storage systems (HDFS, Apache Kudu, Apache HBase, and Amazon S3) and Impala does not include built-in capability for loading local data into these systems.
...
RJDBC has a crude type handling system: columns and column expressions with numeric types are returned to R as `numeric` coulmns, and all other types are returned as `character` columns. This has undesirable effects; for example, Boolean types are returned as `character` columns with values `"0"` for `FALSE` and or `"1"` for `TRUE`.
